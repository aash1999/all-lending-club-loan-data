...
```
## we performed data cleaning and feature engineering to prepare the dataset for predictive modeling. Below are the steps and their significance:

Data Cleaning:

Missing Value Handling: Rows with missing loan_status values were removed as they are essential for target prediction. For numeric columns, missing values were imputed with the median to maintain the dataset's distribution. For categorical columns, missing values were imputed with the mode, ensuring that no important categorical information was lost.
Summary: After cleaning, all datasets retained their original number of rows, indicating that loan_status was complete. Any missing numeric or categorical data was successfully imputed.
Feature Engineering:

Interaction Terms: We created new features by multiplying pairs of numeric columns. This helps the model capture the combined effect of two features, which may be predictive of loan_status.
Polynomial Features: Higher-order terms (e.g., squares, cubes) of numeric features were added to model non-linear relationships. This was done up to the second degree.
Summary: For each dataset, the original 10 features were expanded to include 55 new features, resulting in a total of 69 features per dataset. This significantly enriched the feature space, potentially improving model performance.
