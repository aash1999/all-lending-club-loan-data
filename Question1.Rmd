...
Via the first contingency table, 60 month loans that are fully paid have the highest frequency whereas 60 month loans that are charged off have the lowest frequency. By percentage proportions in the next contingency table, fully paid 60 month loans accounts for about 63% while charged off 36 month loans account for about 12%.
...
```{r, echo=FALSE}
cor(df_final[, c("funded_amnt", "int_rate")], use = "complete.obs")

```

In both cases, the correlation between loan amount and interest rate is bidirectional, indicating that neither variable strongly influences the other. With a correlation coefficient of 0.129, this means that as the loan amount increases, the interest rate tends to increase slightly, but the relationship is not strong (vice versa).
...
Loan Amount: The loan amount does not have a statistically significant effect on the likelihood of a lona being charged off versus fully paid. The higher p-value of 0.71 that is greater than 0.05 indicates that loan amount is not a strong predictor in this model. 
