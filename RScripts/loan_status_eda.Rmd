...
dfr1 <- dfr1 %>% sample_frac(0.3)

# Ensure issue_d is in Date format in df
df1$issue_d <- as.Date(df1$issue_d, format = "%b-%Y")

# Ensure Application Date is in Date format in dfr
dfr1$`Application Date` <- as.Date(dfr1$`Application Date`, format = "%Y-%m-%d")

# Standardize column names
dfr1 <- dfr1 %>%
  rename(issue_d = `Application Date`,  # Rename Application Date to issue_d for consistency
         dti = `Debt-To-Income Ratio`)  # Rename Debt-To-Income Ratio to dti

# Create a source column to distinguish datasets
df1$source <- "Accepted"
dfr1$source <- "Rejected"

# Convert dti to numeric and filter out values greater than 100
df1$dti <- as.numeric(df1$dti)
dfr1$dti <- as.numeric(gsub("%", "", dfr1$dti)) 

# Filter both datasets to remove dti > 100
df1 <- df1 %>% filter(dti <= 100)
dfr1 <- dfr1 %>% filter(dti <= 100)  # Adjusted for percentage conversion

# Combine the two datasets
combined_df <- bind_rows(df1, dfr1)

# Bin the dti values (you can adjust the bin width based on your data)
bin_width <- 0.5  # Adjust as needed for percentage
combined_df <- combined_df %>%
  mutate(dti_bin = cut(dti, breaks = seq(0, 1, by = bin_width), right = FALSE))

# Count the frequency of each dti_bin for each source
dti_freq <- combined_df %>%
  group_by(dti_bin, source) %>%
  summarise(freq = n(), .groups = 'drop')

# Convert dti_bin to numeric for plotting
dti_freq$dti_bin_num <- as.numeric(as.character(gsub("\\[|,.*", "", dti_freq$dti_bin)))

# Plot the line graph showing the frequency of dti values for both sources
ggplot(dti_freq, aes(x = dti_bin_num, y = freq, color = source)) +
  geom_line(size = 1) +
  labs(title = "Frequency of DTI for Accepted and Rejected Loans (DTI <= 100)",
       x = "DTI",
       y = "Frequency",
       color = "Source") +
  scale_x_continuous(labels = scales::percent_format(accuracy = 0.01)) +  # Format x-axis as percentage
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for readability
rm(df1)
rm(dfr1)
...
# Remove NA values (if any) in Debt-To-Income Ratio
dfr1 <- dfr1 %>% filter(!is.na(dti))
...
# Create the distribution plot using ggplot
ggplot(dfr_no_outliers, aes(x = dti)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black", alpha = 0.7) +
  labs(title = "Distribution of Debt-To-Income Ratio (Outliers Removed)",
...
# Apply the function to remove outliers
dfr_no_outliers <- remove_outliers(df1)
