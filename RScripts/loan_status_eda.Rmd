...
df1$issue_d <- as.Date(df1$issue_d)

# Create a period variable
df1$period <- ifelse(df1$issue_d < as.Date("2015-01-01"), "Before 2015", "After 2015")

# Function to create summary data for radar chart
create_radar_data <- function(data) {
  data %>%
    filter(loan_status %in% c("Fully Paid", "Charged Off")) %>%  # Filter for specific loan statuses
    group_by(loan_status) %>%
    summarise(
      median_int_rate = median(int_rate, na.rm = TRUE),
      median_annual_inc = median(annual_inc, na.rm = TRUE),
      median_dti = median(dti, na.rm = TRUE),
      median_fico_range_low = median(fico_range_low, na.rm = TRUE),
      median_fico_range_high = median(fico_range_high, na.rm = TRUE),
      median_revol_util = median(revol_util, na.rm = TRUE),
      .groups = 'drop'  # Prevent warning about grouping
    ) %>%
    pivot_wider(names_from = loan_status, 
                values_from = starts_with("median_"), 
                values_fill = list(median_int_rate = 0, 
                                   median_annual_inc = 0, 
                                   median_dti = 0, 
                                   median_fico_range_low = 0, 
                                   median_fico_range_high = 0, 
                                   median_revol_util = 0),
                names_prefix = "median_")  # Prefix the column names for clarity
}

# Create radar data for both periods
before_radar_data <- create_radar_data(df1[period == "Before 2015"])
after_radar_data <- create_radar_data(df1[period == "After 2015"])

# Function to plot radar chart
plot_radar <- function(data, title) {
  ...
  # Create max and min vectors based on numeric columns
  max_values <- apply(data[, -1], 2, max, na.rm = TRUE)
  min_values <- apply(data[, -1], 2, min, na.rm = TRUE)

  # Combine max, min, and the data
  radar_data <- rbind(max_values, min_values, data[, -1])
  
  # Set row names for the radar chart
  row.names(radar_data) <- c("Max", "Min", as.character(data[[1]]))

  # Plot radar chart with reduced text size
  radarchart(radar_data, axistype = 1,
             ...
             caxislabels = seq(0, 100, 20),  # Customize axis labels
             cex.axis = 0.7,     # Size of axis labels
             cex.main = 0.8,     # Size of title
             pbg = NA)           # Background color for polygon
}

# Plot radar charts
par(mfrow = c(1, 2))  # Set up the plotting area for two plots
plot_radar(before_radar_data, "Before 2015")
plot_radar(after_radar_data, "After 2015")
