...
ggplot() +
  ...
  scale_size_continuous(
    ...
    range = c(0, 10),  # Adjust the range of sizes
...
frequency_data_long <- melt(frequency_data, id.vars = "state", 
                             measure.vars = c("percentage_charged_off_before", "percentage_charged_off_after"), 
                             variable.name = "Period", value.name = "Percentage_Charged_Off")

# Step 2: Create the histograms
ggplot(frequency_data_long, aes(x = Percentage_Charged_Off, fill = Period)) +
  geom_histogram(binwidth = 1, alpha = 0.7, position = "identity", color = "black") +
  scale_fill_manual(values = c("red", "green"), 
                    labels = c("Before Q2 2015", "After Q2 2015")) +
  labs(title = "Distribution of Charged Off Loan Percentages",
       x = "Percentage of Charged Off Loans",
       y = "Frequency",
       fill = "Period") +
  theme_minimal()
...
# Step 5: Calculate charged off loans by state for both periods
charged_off_loans_before <- before_2015_Q2 %>%
  filter(loan_status == "Charged Off") %>%
  group_by(addr_state) %>%
  summarise(charged_off_loans = n(), .groups = "drop")
...
# Step 6: Merge with state coordinates for both periods
frequency_data_before <- state_coords %>%
  left_join(charged_off_loans_before, by = c("state" = "addr_state")) %>%
  mutate(charged_off_loans = ifelse(is.na(charged_off_loans), 0, charged_off_loans))

frequency_data_after <- state_coords %>%
  left_join(charged_off_loans_after, by = c("state" = "addr_state")) %>%
  mutate(charged_off_loans = ifelse(is.na(charged_off_loans), 0, charged_off_loans))

# Step 7: Filter to keep only points within the contiguous US for both datasets
frequency_data_before <- frequency_data_before %>%
  filter(latitude >= 24.396308 & latitude <= 49.384358 & 
         longitude >= -125.0 & longitude <= -66.93457)

frequency_data_after <- frequency_data_after %>%
  filter(latitude >= 24.396308 & latitude <= 49.384358 & 
         longitude >= -125.0 & longitude <= -66.93457)
...
# Step 9: Plot with circles for both periods
ggplot() +
  geom_polygon(data = us_map, aes(x = long, y = lat, group = group), fill = "lightgrey") +
  geom_point(data = frequency_data_before,
             aes(x = longitude, y = latitude, size = charged_off_loans), 
             color = "red", alpha = 0.5) +
  geom_point(data = frequency_data_after,
             aes(x = longitude, y = latitude, size = charged_off_loans), 
             color = "green", alpha = 0.5) +
  scale_size_continuous(
    range = c(8, 50),  # Adjust the range of sizes
    guide = "none"  # Remove the legend for size
  ) +
  labs(title = "Charged Off Loans by State (Before and After Q2 2015)",
       x = "Longitude",
       y = "Latitude") +
  theme_minimal()
