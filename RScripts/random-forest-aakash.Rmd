...
```{r}
train_and_evaluate_rf_cv <- function(train_df, eval_df, target_column, ntree, k_folds = 5) {
  ...
  fold_train_accuracies <- c()
  fold_eval_accuracies <- c()
  ...
  clusterExport(cl, list("formula", "ntree", "target_column"))
  ...
  for (i in 1:k_folds) {
    ...
    fold_eval_idx <- which(folds == i, arr.ind = TRUE)
    fold_train_data <- train_df[-fold_eval_idx, ]
    fold_eval_data <- train_df[fold_eval_idx, ]
    
    
    clusterExport(cl, list("fold_train_data"), envir = environment())
    
    
    rf_model <- parLapply(cl, list(fold_train_data), function(data) {
      randomForest(formula, data = data, importance = TRUE, ntree = ntree)
    })[[1]]
    
    
    fold_train_pred <- predict(rf_model, fold_train_data, type = "response")
    fold_eval_pred <- predict(rf_model, fold_eval_data, type = "response")
    
    fold_train_accuracies <- c(fold_train_accuracies, mean(fold_train_pred == fold_train_data[[target_column]]))
    fold_eval_accuracies <- c(fold_eval_accuracies, mean(fold_eval_pred == fold_eval_data[[target_column]]))
  }
  ...
  cat("Cross-Validation Average Training Accuracy: ", mean(fold_train_accuracies) * 100, "%\n")
  cat("Cross-Validation Average Evaluation Accuracy: ", mean(fold_eval_accuracies) * 100, "%\n")
  
  
  final_rf_model <- randomForest(formula, data = train_df, importance = TRUE, ntree = ntree)
  
  final_train_pred <- predict(final_rf_model, train_df, type = "response")
  final_eval_pred <- predict(final_rf_model, eval_df, type = "response")
  
  final_train_accuracy <- mean(final_train_pred == train_df[[target_column]])
  final_eval_accuracy <- mean(final_eval_pred == eval_df[[target_column]])
  
  cat("Final Model Training Accuracy: ", final_train_accuracy * 100, "%\n")
  cat("Final Model Evaluation Accuracy: ", final_eval_accuracy * 100, "%\n")
  
  # Return results
  return(list(
    cv_avg_train_accuracy = mean(fold_train_accuracies),
    cv_avg_eval_accuracy = mean(fold_eval_accuracies),
...
results <- train_and_evaluate_rf_cv(sample_train_df, sample_eval_df, target_column, ntree, k_folds)
...
for (ntree in ntree_list) {
  ...
  # Train model
  rf_model <- train_and_evaluate_rf_cv(sample_train_df, sample_eval_df, target_column, ntree)
