...
# Function to calculate confidence-adjusted counts
calculate_confidence_counts <- function(count, total, confidence_level) {
  proportion <- count / total
  z <- qnorm(1 - (1 - confidence_level) / 2)
  margin_error <- z * sqrt((proportion * (1 - proportion)) / total)
  
  adjusted_count <- total * (proportion - margin_error)  # Lower bound count at confidence level
  return(max(0, round(adjusted_count))) # Ensure no negative counts
}
...
# Total counts
total_loans <- results$Charged_Off + results$Fully_Paid

# Calculate confidence-adjusted counts for each outcome
outcome_summary <- tibble(
  Outcome = c("Charged Off", "Fully Paid"),
  Count = c(results$Charged_Off, results$Fully_Paid)
) %>%
  rowwise() %>%
  mutate(
    CI_99 = calculate_confidence_counts(Count, total_loans, 0.99),
    CI_95 = calculate_confidence_counts(Count, total_loans, 0.95),
    CI_90 = calculate_confidence_counts(Count, total_loans, 0.90)
  )
