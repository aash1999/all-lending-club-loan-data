...
# Count the number of "Charged Off" and "Fully Paid" loans
loan_status_counts <- table(test_df$loan_status)
...
ggplot(loan_status_df, aes(x = "", y = count, fill = status)) +
  ...
  scale_fill_manual(values = c("Charged Off" = "red", "Fully Paid" = "green")) +  # Customize colors
...
# Call the function
results <- train_and_evaluate_rf(train_df, eval_df, target_column, ntree=120, maxdepth = 25, maxnodes = 20)
...
for (i in 1:N) {
  ...
  # Calculate accuracy for the "Charged Off" class
  accuracy <- calculate_accuracy(results$model, sample_test_df)
...
```{r}
plot_data_melt <- melt(plot_data, id.vars = c("chareoff_class_wt", "fully_paid_class_wt"))

# Heatmap for train_precision_charged_off
ggplot(plot_data_melt[plot_data_melt$variable == "train_precision_charged_off", ], aes(x = fully_paid_class_wt, y = chareoff_class_wt, fill = value)) +
  geom_tile() +
  scale_fill_gradient2(low = "red", high = "green", mid = "yellow", midpoint = 0.8) +
  labs(title = "Train Precision for Charged Off Class", x = "Fully Paid Class Weight", y = "Charged Off Class Weight") +
  theme_minimal()

# Heatmap for train_recall_charged_off
ggplot(plot_data_melt[plot_data_melt$variable == "train_recall_charged_off", ], aes(x = fully_paid_class_wt, y = chareoff_class_wt, fill = value)) +
  geom_tile() +
  scale_fill_gradient2(low = "red", high = "green", mid = "yellow", midpoint = 0.8) +
  labs(title = "Train Recall for Charged Off Class", x = "Fully Paid Class Weight", y = "Charged Off Class Weight") +
  theme_minimal()

# Heatmap for eval_precision_charged_off
ggplot(plot_data_melt[plot_data_melt$variable == "eval_precision_charged_off", ], aes(x = fully_paid_class_wt, y = chareoff_class_wt, fill = value)) +
  geom_tile() +
  scale_fill_gradient2(low = "red", high = "green", mid = "yellow", midpoint = 0.8) +
  labs(title = "Eval Precision for Charged Off Class", x = "Fully Paid Class Weight", y = "Charged Off Class Weight") +
  theme_minimal()


```
```{r}
# Assuming `plot_data` is your dataframe with the necessary columns

# Step 1: Compute a weighted score for each row based on precision and recall.
# You can assign different weights to precision and recall based on their importance.
# For example, we can give a 50-50 weight to both precision and recall.

plot_data$precision_recall_score <- 0.7 * (plot_data$eval_recall_charged_off) + 0.3 * (plot_data$eval_precision_fully_paid)


# Step 2: Find the row with the highest weighted score.
optimal_row <- plot_data[which.max(plot_data$precision_recall_score), ]

# Step 3: Extract the optimal class weights
optimal_charged_off_class_wt <- optimal_row$chareoff_class_wt
optimal_fully_paid_class_wt <- optimal_row$fully_paid_class_wt

# Step 4: Print the optimal class weights
cat("Optimal Charged Off Class Weight: ", optimal_charged_off_class_wt, "\n")
cat("Optimal Fully Paid Class Weight: ", optimal_fully_paid_class_wt, "\n")
